...
These two constraints motivate the software-only approach of Section~\ref{sec:math}, rather than a hardware upgrade (Section~\ref{sec:limitations}).
...
Figure~\ref{fig:pipeline-new} gives a visual overview of the pipeline described above, spanning three main blocks: (i) 3D reconstruction, (ii) attaching YOLO semantic labels, and (iii) distance estimation.

\begin{figure*}[t]
\centering
\includegraphics[width=0.66\textwidth]{paper_figs/hinh1_pipeline.drawio.pdf}
...
\textbf{Distance-size ambiguity.} Formula (3) assumes a known height $H$ for the target class. Two objects of the same class with different true heights $H_1 \neq H_2$ at distances $d_1, d_2$ satisfying
\begin{equation}
\frac{H_1}{d_1} = \frac{H_2}{d_2} \;\left(= \frac{h_{\text{px}}}{f_y}\right),
\end{equation}
produce the same $h_{\text{px}}$ and cannot be distinguished by Eq.~(3) alone — a limitation inherent to monocular imaging that MapAnything's depth inference does not resolve either (Section~\ref{sec:global-fit-stable}). DBSCAN clustering at S6 mitigates confusion with transient false detections by favoring the densest, most persistent cluster, but not confusion between two co-located instances of the same class; the method assumes a single target instance, consistent with the current setup (Section~\ref{sec:limitations}).

None of the quantities above depend on the pose odometry of a single frame, except for $\mathbf{o}$ (reference results) — the alignment uses the entire trajectory, while distance estimation does not use odometry at all.
...
Each capture session follows the B1--B8 procedure of Section~\ref{sec:math}. The method is validated over six independent sessions at \SI{3.00}{\meter} (measured with a measuring tape), the robot observing the plant pot from a different trajectory each time. To evaluate stability at greater range, the process is repeated at \SI{5}{\meter} and \SI{7}{\meter}, five sessions each, combined with the \SI{3}{\meter} results in Table~\ref{tab:semantic-bearing-results} and analyzed separately in Section~\ref{sec:distance-eval}. Figure~\ref{fig:room-scan} illustrates the experimental environment from a full-room scan (Section~\ref{sec:room-scan}).

\begin{figure*}[t]
\centering
\begin{subfigure}[b]{0.46\textwidth}
\centering
\includegraphics[height=3.2cm,keepaspectratio]{paper_figs/room_scan_photo.pdf}
\caption{Real-world captured image}
\end{subfigure}
\hspace{0.6em}
\begin{subfigure}[b]{0.46\textwidth}
\centering
\includegraphics[height=3.2cm,keepaspectratio]{paper_figs/room_scan_topdown.pdf}
...
Table~\ref{tab:semantic-bearing-results} summarizes the results per session along with three quality indicators: YOLO detection confidence at the distance-estimation frame, the corresponding bounding-box height, and the number of points in the largest semantic cluster used for orientation.

Six sessions show errors in the range $2$--$9\%$; no session exceeds \SI{0.26}{\meter}. YOLO confidence varies considerably (0.63--0.90) while semantic cluster size stays high ($>$29\,700/30\,000 points, over 99\% of sampled points), showing that MapAnything reconstructs a contiguous, largely unfragmented point cluster regardless of detection confidence.

\textbf{Correlation between input quality and error.} The Pearson correlation is $r=0.34$ (YOLO confidence) and $r=0.20$ (cluster size) against absolute error — both weak and statistically insignificant at $n=6$, consistent with Section~\ref{sec:global-fit-stable}: accuracy comes from separating the two signals from per-frame pose odometry, not from high-confidence detection or large clusters.
...
\centering
\includegraphics[height=2.1cm,keepaspectratio]{paper_figs/anh_thuc_te.pdf}
\caption{Experimental environment}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\columnwidth}
\centering
\includegraphics[height=2.1cm,keepaspectratio]{paper_figs/fig_yolo_detection_single.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\columnwidth]{paper_figs/figure5.pdf}
...
\centering
\includegraphics[width=\textwidth,height=2.3cm,keepaspectratio]{paper_figs/paperfig_new36.pdf}
\caption{Session 1 --- \SI{2.77}{\meter}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth,height=2.3cm,keepaspectratio]{paper_figs/paperfig_new46.pdf}
\caption{Session 2 --- \SI{2.76}{\meter}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth,height=2.3cm,keepaspectratio]{paper_figs/paperfig_new47.pdf}
...
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth,height=2.3cm,keepaspectratio]{paper_figs/paperfig_new48.pdf}
\caption{Session 4 --- \SI{3.08}{\meter}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth,height=2.3cm,keepaspectratio]{paper_figs/paperfig_new49.pdf}
\caption{Session 5 --- \SI{3.26}{\meter}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth,height=2.3cm,keepaspectratio]{paper_figs/paperfig_new50.pdf}
...
\item \textbf{ArUco:} A \SI{15}{\centi\meter} marker gave \texttt{solvePnP} errors of \SIrange{0.3}{0.7}{\meter} beyond \SI{2}{\meter} or under oblique views --- as large as the drift it was meant to correct.
\item \textbf{EKF (\texttt{/odometry/filtered}):} covariance diverged to $\sim 10^{22}$, unusable without further tuning.
\end{itemize}
